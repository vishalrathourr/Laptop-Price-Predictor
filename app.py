import streamlit as st 
import pickle
import numpy as np 
from PIL import Image

image = Image.open('pngtree-laptop-icon-png-image_66.png')

st.sidebar.image(image,output_format='PNG', width=200)
st.sidebar.markdown("<hr>", unsafe_allow_html=True)
st.sidebar.subheader("Please provide the configuration details of the laptop for which you'd like to predict the price.")

# import the model
pipe = pickle.load(open('pipe.pkl','rb'))
df = pickle.load(open('df.pkl','rb'))

st.title('Laptop Price Predictor')

# Brand
company = st.selectbox('Brand',df['Company'].unique())

# type of laptop
type = st.selectbox('Type',df['TypeName'].unique())

# Ram
ram = st.selectbox('RAM(in GB)',[2,4,6,8,12,24,32,64])

#weight
weight = st.number_input('Weight of the laptop')

# Touchscreen 
# touchscreen = st.selectbox('TouchScreen',['No','Yes'])
touchscreen = st.toggle('TouchScreen')

#IPS
# ips = st.selectbox('IPS',['No','Yes'])
ips = st.toggle('IPS')

# Screen size
screen_size = st.number_input('Screen Size')

# screen resolution
resolution = st.selectbox('Screen Resolution',['1920x1080','1366x768','1600x900','3840x2160','3200x1800','2880x1800','2560x1600','2560x1440','2304x1440'])

X_res = int(resolution.split('x')[0])
Y_res = int(resolution.split('x')[1])

#cpu
processor = st.selectbox('Processor',df['Processor'].unique())

hdd = st.selectbox('HDD(in GB)',[0,128,256,512,1024,2048])

ssd = st.selectbox('SSD(in GB)',[0,8,128,256,512,1024])

gpu = st.selectbox('GPU',df['Gpu brand'].unique())

os = st.radio('OS',df['os'].unique())


if st.button('Predict Price'):
    # query
    if touchscreen == True:
        touchscreen = 1
    else:
        touchscreen = 0
        
    if ips == True:
        ips = 1
    else:
        ips = 0
    
    ppi = ((X_res**2) + (Y_res**2))**0.5/screen_size
        
    query = np.array([company, type, ram, weight, touchscreen,ips, ppi, processor, hdd, ssd, gpu, os])
    query = query.reshape(1,12)
    st.title("The predicted price of this configuration is ₹" + str(int(np.exp(pipe.predict(query)[0]))))
    # st.title(pipe.predict(query))

